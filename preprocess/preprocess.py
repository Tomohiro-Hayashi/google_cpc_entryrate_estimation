...
if args[1] == 'train':
    for name in type_str:
        ...
        table.to_csv('preprocess/' + new_col + str('.csv'))
elif args[1] == 'scoring':
    for name in type_str:
        new_col = name + str('_encode')
        table = pd.read_csv('preprocess/' + new_col + str('.csv'))
...
# ##スケーリング
# for name in feature:
#     feature[name] = ( feature[name] - min(feature[name]) ) *1.0 /(max(feature[name]) - min(feature[name]))

#データを出力する
if args[1] == 'train':
    explain.append('entry_flg')
    data[explain].to_csv("preprocess/preprocess.csv")
    # feature.to_csv("preprocess_pca.csv")
elif args[1] == 'scoring':
    data[explain].to_csv("preprocess/preprocess_scoring.csv")
    # feature.to_csv("preprocess_scoring_pca.csv")
